...
async def _create_batch_and_products(client, batch_number: int, codes: list[str]) -> int:
    ...
    if codes:
        await client.post(f"/api/v1/batches/{batch_id}/aggregate", json={"unique_code": codes[0]})
...
async def test_generate_batch_report_excel(client, monkeypatch):
    ...
    batch_id = await _create_batch_and_products(client, batch_number=401, codes=["R1", "R2"])
