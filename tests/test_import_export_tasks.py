...
def _upload_import_file(rows: list[list], extension: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"pytest_import_{uuid.uuid4().hex[:8]}.{extension}"
    )
    ...
    storage.client.fput_object(
        bucket_name="imports", object_name=object_name, file_path=local_path
    )
...
async def test_import_batches_from_excel(monkeypatch):
    ...
    result = await imports_module._import_batches_from_file_async(
        object_name, user_id=None
    )
