...
async def _generate_batch_report_async(batch_id: int) -> dict:
    async with AsyncSessionLocal() as session:
        ...
        wb = _build_report_workbook(batch)

        file_name = f"batch_{batch_id}_report_{uuid.uuid4().hex[:8]}.xlsx"
...
def generate_batch_report(self, batch_id: int, format: str = "excel", user_email: str | None = None) -> dict:
    return asyncio.run(_generate_batch_report_async(batch_id))
